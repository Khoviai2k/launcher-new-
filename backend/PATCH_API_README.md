...
- **Premium patches**: Yêu cầu quyền VIP để truy cập
...
```javascript
{
  ...
  requires_vip: Boolean,  // Yêu cầu VIP (default: true)
  is_free: Boolean,       // Miễn phí (default: false)
  free_until: Date,       // Miễn phí đến khi nào
...
1. **Free patches** (`is_free: true`): Ai cũng có thể truy cập
2. **Temporary free** (`free_until > now`): Miễn phí tạm thời
3. **Non-VIP required** (`requires_vip: false`): Không yêu cầu VIP
4. **VIP required** (`requires_vip: true`): Yêu cầu VIP active

### VIP Status Check:
```javascript
// User phải có VIP active để truy cập premium patches
user.vip_status === 'active' && 
user.vip_expiry && 
user.vip_expiry > new Date()
```
